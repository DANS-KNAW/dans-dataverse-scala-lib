...
import nl.knaw.dans.lib.scaladv.model.DataverseMessage
import nl.knaw.dans.lib.scaladv.model.dataset.MetadataFieldSerializer
import nl.knaw.dans.lib.scaladv.model.search.ResultItemSerializer
 ...
 * provides easy access to that structure. If JSON is returned but it is not modelled by a case class in [[nl.knaw.dans.lib.scaladv.model]],
 ...
 */
case class DataverseResponse[D: Manifest] private[scaladv](httpResponse: HttpResponse[Array[Byte]]) {
   ...
   *
   * @see [[nl.knaw.dans.lib.scaladv.model]]
   */
  def data: Try[D] = {
    json
