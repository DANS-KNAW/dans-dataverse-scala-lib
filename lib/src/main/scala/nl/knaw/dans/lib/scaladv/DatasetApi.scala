...
import nl.knaw.dans.lib.scaladv.model._
import nl.knaw.dans.lib.scaladv.model.dataset.UpdateType.UpdateType
import nl.knaw.dans.lib.scaladv.model.dataset.{ DatasetLatestVersion, DatasetVersion, Embargo, FieldList, FileList, MetadataBlock, MetadataBlocks, PrivateUrlData }
 ...
 *
 */
class DatasetApi private[scaladv](datasetId: String, isPersistentDatasetId: Boolean, configuration: DataverseInstanceConfig, workflowId: Option[String] = None) extends TargetedHttpSupport with DebugEnhancedLogging {
   ...
   * @param replace replace existing values if `true` only add values otherwise
   * @return
   */
