...
object DeactivateBannerMessage extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val id = args(0).toInt
  val result = for {
    response <- server.admin().deactivateBannerMessage(id)
