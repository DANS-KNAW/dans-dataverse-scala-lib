...
object GetGroup extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val groupAlias = args(0)

  private val result = for {
    response <- server.dataverse("root").getGroup(groupAlias)
    ...
    group <- response.data
    _ = logger.info(s"Group display name = ${group.displayName}")
  } yield ()
