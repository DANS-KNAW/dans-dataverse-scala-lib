...
object GetDatabaseSetting extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  val setting = args(0)
  val result = for {
    response <- server.admin().getDatabaseSetting(setting)
    ...
    msg <- response.data
    _ = logger.info(s"$setting = ${ msg.message }")
  } yield ()
