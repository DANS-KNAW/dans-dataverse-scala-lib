...
import nl.knaw.dans.lib.dataverse.model.Group
import nl.knaw.dans.lib.logging.DebugEnhancedLogging
import org.json4s.native.Serialization
import org.json4s.{ DefaultFormats, Formats }

object UpdateGroup extends App with DebugEnhancedLogging with BaseApp {
  ...
  private val result = for {
    response <- server.dataverse("root").updateGroup(groupAlias, group)
