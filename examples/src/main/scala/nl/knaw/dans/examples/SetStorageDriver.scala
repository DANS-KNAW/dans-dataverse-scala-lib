...
object SetStorageDriver extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val persistentId = args(0)
  private val driver = args(1)

  val result = for {
    response <- server.dataset(persistentId).setStorageDriver(driver)
