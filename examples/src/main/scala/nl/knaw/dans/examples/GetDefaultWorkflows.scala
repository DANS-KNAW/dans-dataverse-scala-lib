...
object GetDefaultWorkflows extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats

  val result = for {
    response <- server.admin().getDefaultWorkflows
    ...
    _ = logger.info(s"Default workflows: $workflows")
  } yield ()
