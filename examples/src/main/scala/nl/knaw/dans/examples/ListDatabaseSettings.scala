...
object ListDatabaseSettings extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  val result = for {
    response <- server.admin().listDatabaseSettings()
    ...
    _ = logger.info(s"keys: ${settings.keySet.mkString(", ")}")
  } yield ()
