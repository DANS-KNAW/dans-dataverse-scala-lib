...
object GetDatasetStorageSize extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val persistentId = args(0)
  private val includeCached = if (args.length > 1) args(1).toBoolean
                              else false

  val result = for {
    response <- server.dataset(persistentId).getStorageSize(includeCached)
