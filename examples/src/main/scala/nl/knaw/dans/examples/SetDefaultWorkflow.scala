...
object SetDefaultWorkflow extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val triggerType = args(0)
  private val workflowId = args(1)

  val result = for {
    response <- server.admin().setDefaultWorkflow(triggerType, workflowId.toInt)
