...
object AddRoleAssigneeToGroup extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val groupAlias = args(0)
  private val assignee = args(1)

  private val result = for {
    response <- server.dataverse("root").addRoleAssigneesToGroup(groupAlias, List(assignee))
