...
import nl.knaw.dans.lib.dataverse.model.Workflow
import nl.knaw.dans.lib.logging.DebugEnhancedLogging
import org.json4s.native.Serialization
import org.json4s.{ DefaultFormats, Formats }

object AddWorkflow extends App with DebugEnhancedLogging with BaseApp {
  ...
  val result = for {
    response <- server.admin().addWorkflow(workflow)
