...
object GetGroups extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats

  private val result = for {
    response <- server.dataverse("root").listGroups()
    ...
    _ = logger.info(s"Group IDs: ${ groups.map(_.identifier).mkString(", ") }")
  } yield ()
