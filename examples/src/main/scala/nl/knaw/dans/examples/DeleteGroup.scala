...
object DeleteGroup extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val groupAlias = args(0)

  private val result = for {
    response <- server.dataverse("root").deleteGroup(groupAlias)
    ...
    msg <- response.data
    _ = logger.info(s"msg = ${ msg.message }")
  } yield ()
