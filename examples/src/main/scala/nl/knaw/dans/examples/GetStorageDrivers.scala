...
object GetStorageDrivers extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats

  val result = for {
    response <- server.admin().getStorageDrivers
    ...
    _ = logger.info(s"Driver labels: ${drivers.keys.mkString(", ")}")
  } yield ()
