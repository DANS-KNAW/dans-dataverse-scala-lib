...
object SetDefaultRole extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val dataverse = args(0)
  private val role = args(1)

  val result = for {
    defaultRole <- Try { DefaultRole.withName(role) }
    response <- server.dataverse(dataverse).setDefaultRole(defaultRole)
